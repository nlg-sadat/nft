...
export class Web3Service {
  ...
  async getMintingPrice(length: number): Promise<string> {
    try {
      ...
      const price = await contract.read.getMintingPrice([BigInt(length)])
  ...
  async isUsernameTaken(username: string): Promise<boolean> {
    try {
      ...
      return await contract.read.isUsernameTaken([username])
    } catch (error) {
  ...
  async isValidUsername(username: string): Promise<boolean> {
    try {
      ...
      return await contract.read.isValidUsername([username])
    } catch (error) {
  ...
  async hasMinted(address: string): Promise<boolean> {
    try {
      ...
      return await contract.read.hasMinted([address as Address])
    } catch (error) {
  ...
  async mintUsername(username: string): Promise<{ hash: Hash; wait: () => Promise<TransactionReceipt> }> {
    ...
    try {
      ...
      const price = await contract.read.getMintingPrice([BigInt(username.length)])
  ...
  async getUsernameByAddress(address: string): Promise<string> {
    ...
    return await contract.read.getUsernameByAddress([address as Address])
  }
  ...
  async getUserInfo(username: string): Promise<{ userAddress: string; username: string; tokenId: string }> {
    ...
    const userInfo = await contract.read.getUserInfo([username])
    return {
  ...
  async checkTokenApproval(
    ...
    try {
      ...
      const [decimals, allowance] = await Promise.all([
        ...
        contract.read.allowance([ownerAddress as Address, spenderAddress as Address]),
      ])
